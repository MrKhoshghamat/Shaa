...
var KTCreateLaboratory = function () {
    ...
    return {
        init: function () {
            ...
            var load_step_content = function () {
                const sel = '#stepper-content-' + r.getCurrentStepIndex();
                if (!$(sel).hasClass('init')) {

                    switch (r.getCurrentStepIndex()) {
                        case 1: { $(sel).html('step 1 load at ' + new Date().toString()); return; }
                        case 2: { $(sel).html('step 2 load at ' + new Date().toString()); return; }
                        case 3: { $(sel).html('step 3 load at ' + new Date().toString()); return; }
                        case 4: { $(sel).html('step 4 load at ' + new Date().toString()); return; }
                        default:
                    }
            ...
            var kt_stepper_changed = function (e) {
